...
(* Parse, reporting syntax errors with location *)
...
(* Pretty-printer for AST (unchanged) *)
let rec string_of_expr = function
  ...
  | LoopExpr (While (cond, _)) -> "while " ^ string_of_expr cond ^ " {...}"
  | LoopExpr (DoWhile (_, cond)) -> "do {...} while " ^ string_of_expr cond
...
and string_of_stmt = function
  ...
  | FunctionDef fd  ->
      "fun " ^ fd.fname ^ "(" ^ String.concat ", " fd.params ^ ") { ... }"
...
let () =
  ...
  let sast =
    try
      let s = Semant.check_program ast in
      s
    with
    | Semant.SemanticError msg ->
        Printf.eprintf "Type error: %s\n" msg;
        exit 1
  in

  (* 3) Generate and print LLVM IR *)
  let ir = Codegen.translate_program sast in
  print_endline ir
